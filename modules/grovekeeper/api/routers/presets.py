# =============================================================================
# routers/presets.py — Grovekeeper module: saved tree presets
# thrive module `grovekeeper`
#
...
router = APIRouter(prefix="/grovekeeper/presets", tags=["grovekeeper"])
...
def init_db():
    ...
    try:
        db.execute("""
            CREATE TABLE IF NOT EXISTS grovekeeper_presets (
...
def list_presets(db=Depends(get_db)):
    rows = db.execute(
        "SELECT id, name, data, created_at FROM grovekeeper_presets ORDER BY name"
    ).fetchall()
...
def save_preset(body: PresetIn, db=Depends(get_db)):
    ...
    existing = db.execute("SELECT id FROM grovekeeper_presets WHERE name=?", (name,)).fetchone()
    if existing:
        db.execute("UPDATE grovekeeper_presets SET data=? WHERE id=?", (payload, existing["id"]))
        db.commit()
        return {"id": existing["id"], "name": name, "data": body.data}
    cur = db.execute("INSERT INTO grovekeeper_presets (name, data) VALUES (?, ?)", (name, payload))
    db.commit()
...
def delete_preset(preset_id: int, db=Depends(get_db)):
    db.execute("DELETE FROM grovekeeper_presets WHERE id=?", (preset_id,))
    db.commit()
